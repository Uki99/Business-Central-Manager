...
$var_CheckForUpdatesBtn.Add_Click({
    ...
    try {
        Update-BCManagerApplication -owner $owner -repo $repo -currentVersion $settings.settings.ApplicationVersion -upToDateMessage $true -window $window -closeWindow $true -Icon $Icon
    } catch {
        $errorMessage = $_.ToString()
        [System.Windows.Forms.MessageBox]::Show($errorMessage, "Error", "OK", "Error")
        Exit
